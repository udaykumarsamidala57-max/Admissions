...
<div class="container">
    ...
    <div class="table-box">
        ...
        <table>
            ...
            <tbody>
                <%
                int tDC=0, tDP=0, tDV=0, tCGR=0, tPGR=0, tVGR=0, tCBR=0, tPBR=0, tVBR=0, tTC=0, tTP=0, tTV=0;

                for(String cls : classOrder) {
                    int[] cap = capacityMap.getOrDefault(cls, new int[]{0,0,0,0}); // [day, gCap, bCap, total]
                    int[] ps = dashboardMatrix.getOrDefault(cls, new int[]{0,0,0,0,0,0,0}); // [day, gP, bP, semi, eD, eR, eS]

                    int dCap = cap[0], gCap = cap[1], bCap = cap[2], totCap = cap[3];
                    int dPres = ps[0] + ps[3]; // Day + Semi
                ...
                <tr>
                    ...
                    <td><%= dCap %></td><td><%= dPres %></td><td class="<%= dVac < 0 ? "neg":"" %>"><%= dVac %></td>
                    ...
                    <td class="<%= totVac < 0 ? "neg":"" %>" style="background:#f1f5f9;"><%= totVac %></td>
                </tr>
                <% } %>
                <tr class="grand">
                    <td>GRAND TOTAL</td>
